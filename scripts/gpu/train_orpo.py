#!/usr/bin/env python3
"""
GPU ORPO Training Script
Odds Ratio Preference Optimization - combines SFT and preference alignment in one step.
Better for class-imbalanced preference data and long-tail domains.

ORPO doesn't require a reference model and includes an SFT component,
making it efficient for single-stage training from base models.

Usage:
    python scripts/gpu/train_orpo.py --config configs/gpu/orpo_config.yaml

    # With DeepSpeed
    accelerate launch --config_file configs/gpu/deepspeed/ds_zero2.json \
        scripts/gpu/train_orpo.py --config configs/gpu/orpo_config.yaml
"""
    ...
    try:
        ...
        if not run_name:
            run_name = wandb_config.get(
                "run_name",
                f"orpo-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
            )

        wandb.init(
            project=wandb_config.get("project", "llm-finetuning"),
            name=run_name,
            tags=wandb_config.get("tags", ["orpo"]),
            config=config,
            resume="allow",
        )
...
def load_preference_dataset(config: dict):
    """Load preference pairs for ORPO training."""
...
def train(config: dict):
    """Main ORPO training function."""
    from trl import ORPOConfig, ORPOTrainer

    console.print(Panel.fit(
        "[bold green]GPU ORPO Training[/bold green]\n"
        f"Model: {config.get('model', {}).get('name', 'N/A')}\n"
        f"Beta: {config.get('orpo', {}).get('beta', 0.1)}",
    ...
    training_args = ORPOConfig(
        output_dir=train_config.get("output_dir", "outputs/gpu/checkpoints/orpo"),
        # ORPO specific
        beta=orpo_config.get("beta", 0.1),  # Odds ratio weight
        max_length=orpo_config.get("max_length", 2048),
        max_prompt_length=orpo_config.get("max_prompt_length", 1024),
        # Training
        ...
        # Optimizer - ORPO can use higher LR than DPO since it includes SFT
        learning_rate=train_config.get("learning_rate", 8e-6),
    ...
    console.print("\n[bold]ORPO Configuration:[/bold]")
    console.print(f"  Beta (odds ratio weight): {orpo_config.get('beta', 0.1)}")
    console.print(f"  Combines SFT + preference alignment")
    console.print(f"  No reference model required")
    console.print(f"  Learning rate: {train_config.get('learning_rate', 8e-6)}")
    ...
    console.print(f"\n[bold green]ORPO training complete![/bold green]")
...
def main():
    parser = argparse.ArgumentParser(
        description="GPU ORPO Training with TRL",
...
ORPO advantages:
    - Combines SFT + preference in single stage
    - No reference model required
    - Better for class-imbalanced data
    - Can train from base model directly
        """
    )
