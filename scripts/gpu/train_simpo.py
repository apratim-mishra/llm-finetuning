#!/usr/bin/env python3
"""
GPU SimPO Training Script
Simple Preference Optimization - reference-free alternative to DPO.
Better for noisy preference data and stable training.

SimPO uses length-normalized log probabilities and doesn't require a reference model,
making it more memory-efficient and stable than DPO.

Usage:
    python scripts/gpu/train_simpo.py --config configs/gpu/simpo_config.yaml

    # With DeepSpeed
    accelerate launch --config_file configs/gpu/deepspeed/ds_zero2.json \
        scripts/gpu/train_simpo.py --config configs/gpu/simpo_config.yaml
"""
    ...
    try:
        ...
        if not run_name:
            run_name = wandb_config.get(
                "run_name",
                f"simpo-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
            )

        wandb.init(
            project=wandb_config.get("project", "llm-finetuning"),
            name=run_name,
            tags=wandb_config.get("tags", ["simpo"]),
            config=config,
            resume="allow",
        )
...
def load_preference_dataset(config: dict):
    """Load preference pairs for SimPO training."""
...
def train(config: dict):
    ...
    console.print(Panel.fit(
        "[bold green]GPU SimPO Training[/bold green]\n"
        f"Model: {config.get('model', {}).get('name', 'N/A')}\n"
        f"Beta: {config.get('simpo', {}).get('beta', 2.5)}\n"
        f"Gamma/Beta ratio: {config.get('simpo', {}).get('simpo_gamma', 0.5)}",
    ...
    # SimPO uses CPOConfig with loss_type="simpo"
    training_args = CPOConfig(
        output_dir=train_config.get("output_dir", "outputs/gpu/checkpoints/simpo"),
        # SimPO specific
        loss_type="simpo",
        beta=simpo_config.get("beta", 2.5),  # Higher beta than DPO
        simpo_gamma=simpo_config.get("simpo_gamma", 0.5),  # Target margin ratio
        cpo_alpha=simpo_config.get("cpo_alpha", 1.0),  # CPO mixing coefficient
        max_length=simpo_config.get("max_length", 2048),
        max_prompt_length=simpo_config.get("max_prompt_length", 1024),
        # Training
    ...
    # SimPO doesn't need a reference model (more memory efficient)
    trainer = CPOTrainer(
        model=model,
        args=training_args,
    ...
    console.print("\n[bold]SimPO Configuration:[/bold]")
    console.print(f"  Beta: {simpo_config.get('beta', 2.5)}")
    console.print(f"  Gamma (target margin): {simpo_config.get('simpo_gamma', 0.5)}")
    console.print(f"  No reference model (memory efficient)")
    console.print(f"  Learning rate: {train_config.get('learning_rate', 5e-7)}")

    console.print("\n[bold]Starting SimPO training...[/bold]")
    ...
    console.print(f"\n[bold green]SimPO training complete![/bold green]")
...
def main():
    parser = argparse.ArgumentParser(
        description="GPU SimPO Training with TRL",
...
SimPO advantages over DPO:
    - No reference model required (saves ~50% memory)
    - More stable with noisy preference data
    - Length-normalized rewards prevent length exploitation
        """
    )
